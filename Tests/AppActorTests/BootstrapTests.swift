...
private actor FinishedTransactionRecorder {
...
final class BootstrapTests: XCTestCase {
    ...
    func testSyncReceiptWithSyncedOriginalTransactionIdFinishesCoalescedUnfinished() async throws {
        storage.setAppUserId("current_user")
        let queueStore = InMemoryPaymentQueueStore()
        let processor = AppActorPaymentProcessor(store: queueStore, client: mockClient)
        let watcher = AppActorTransactionWatcher(
        ...
        let skippedTransactionIds = ["tx_skipped_1", "tx_skipped_2"]
        skippedTransactionIds.forEach {
            queueStore.upsert(makeQueueItem(appUserId: "current_user", transactionId: $0, phase: .needsFinish))
        }

        let finishRecorder = FinishedTransactionRecorder()
        for transactionId in skippedTransactionIds {
            await watcher.recordCoalescedUnfinished(
                originalTransactionId: "orig_sync_cleanup",
                transactionId: transactionId,
                finish: {
                    await finishRecorder.append(transactionId)
                }
            )
        }

        let config = AppActorPaymentConfiguration(
            apiKey: "pk_test_synced_original_cleanup",
            baseURL: URL(string: "https://api.test.appactor.com")!
        )
        appactor.configureForTesting(config: config, client: mockClient, storage: storage)
        appactor.paymentQueueStore = queueStore
        ...
        await appactor.handleReceiptCustomerInfoUpdate(
            AppActorCustomerInfo(appUserId: "current_user"),
            receiptContext: AppActorReceiptCustomerUpdateContext(
                appUserId: "current_user",
                productId: "com.test.monthly",
                sourceIntent: .sync,
                originalTransactionId: "tx_representative",
                syncedOriginalTransactionId: "orig_sync_cleanup",
        ...
        for transactionId in skippedTransactionIds {
            XCTAssertTrue(
                queueStore.isPosted(key: AppActorPaymentQueueItem.makeKey(transactionId: transactionId)),
                "Coalesced skipped transaction should be marked posted"
            )
        }
        XCTAssertTrue(queueStore.allItems().isEmpty, "Coalesced skipped queue items should be reconciled")
        let finishedSnapshot = await finishRecorder.snapshot().sorted()
        XCTAssertEqual(finishedSnapshot, skippedTransactionIds.sorted())
    }

    func testOldIdentitySyncReceiptStillFinishesCoalescedUnfinished() async throws {
        storage.setAppUserId("current_user")
        let queueStore = InMemoryPaymentQueueStore()
        let processor = AppActorPaymentProcessor(store: queueStore, client: mockClient)
        let watcher = AppActorTransactionWatcher(
        ...
        let skippedTransactionIds = ["tx_skipped_old_1", "tx_skipped_old_2"]
        skippedTransactionIds.forEach {
            queueStore.upsert(makeQueueItem(appUserId: "old_user", transactionId: $0, phase: .needsFinish))
        }

        let finishRecorder = FinishedTransactionRecorder()
        for transactionId in skippedTransactionIds {
            await watcher.recordCoalescedUnfinished(
                originalTransactionId: "orig_sync_cleanup_old_user",
                transactionId: transactionId,
                finish: {
                    await finishRecorder.append(transactionId)
                }
        ...
        mockClient.getCustomerHandler = { appUserId, _ in
            .fresh(
                ...
                eTag: nil,
                requestId: "req_current_customer_refresh",
                signatureVerified: false
            )
        }

        let config = AppActorPaymentConfiguration(
            apiKey: "pk_test_synced_original_cleanup_old_user",
            baseURL: URL(string: "https://api.test.appactor.com")!
        )
        appactor.configureForTesting(config: config, client: mockClient, storage: storage)
        appactor.paymentQueueStore = queueStore
        ...
        await appactor.handleReceiptCustomerInfoUpdate(
            AppActorCustomerInfo(appUserId: "old_user"),
            receiptContext: AppActorReceiptCustomerUpdateContext(
                appUserId: "old_user",
                productId: "com.test.monthly",
                sourceIntent: .sync,
                originalTransactionId: "tx_representative_old_user",
                syncedOriginalTransactionId: "orig_sync_cleanup_old_user",
                clientPurchaseContext: AppActorClientPurchaseContext(
                    clientObservedAt: Date(),
                    clientDeliverySource: .foregroundSync
                )
        ...
        XCTAssertEqual(
            ...
            baselineCustomerFetches + 1,
            "An old-identity sync receipt should still refresh the current user"
        )
        for transactionId in skippedTransactionIds {
            XCTAssertTrue(
                queueStore.isPosted(key: AppActorPaymentQueueItem.makeKey(transactionId: transactionId)),
                "Coalesced skipped transaction should be marked posted even after identity changes"
            )
        }
        XCTAssertTrue(queueStore.allItems().isEmpty, "Coalesced skipped queue items should be reconciled")
        let finishedSnapshot = await finishRecorder.snapshot().sorted()
