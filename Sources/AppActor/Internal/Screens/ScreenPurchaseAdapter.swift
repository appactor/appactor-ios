...
/// Turns the screen's four-state purchase contract into SDK calls and back.
/// Not mechanical in one place: `server_confirmed` has no counterpart in
/// ``AppActorPurchaseResult``, and it is what the contract exists for.
@MainActor
final class AppActorScreenPurchaseAdapter: AppActorScreenPurchaseGateway {

    /// Inside the runtime's own confirmation timer, so when this gives up the
    /// runtime is still listening and the screen releases its controls on a
    /// decision rather than on a timeout at both ends.
    ...
    func purchase(packageId: String) async -> AppActorScreenPurchaseOutcome {
        ...
        do {
            ...
            let result = try await AppActor.shared.purchase(package: package, placement: lookupKey)

            switch result {
            case .success(let customerInfo, let purchaseInfo):
                // Rule #13. `.success` alone does not mean the server agreed:
                // on a receipt-POST timeout the SDK computes entitlements from
                // StoreKit and succeeds anyway, receipt still queued. Reporting
                // that as plain `completed` is how a screen says "Premium
    ...
    ///
    /// Two signals, because neither settles it alone: the **receipt queue**,
    /// polled until the item leaves it (dead-lettered is a definite rejection,
    /// but a `permanent_error` with `finishTransaction: true` also removes it,
    /// so disappearing proves nothing), and the **server**, asked once at the
    /// end for whether the entitlement is actually active.
    ///
    /// Undecided returns `.unknown` and sends nothing: claiming "failed" over a
    /// receipt that may post a minute later is worse than saying nothing.
    func awaitServerConfirmation(transactionId: String) async -> AppActorScreenConfirmation {
        // Through the processor, never its store: the store is `@unchecked
        // Sendable` on the contract that only the `AppActorPaymentProcessor`
        // actor touches it, and polling it from the MainActor while the drain
        // loop removes the same item is a data race -- ThreadSanitizer
        // confirmed -- right after the user paid.
        ...
        // Read the product while the receipt is still queued: the pipeline
        // removes the item when it finishes, and that removal is exactly why
        // the server has to be asked below.
        ...
        // No mapping: an older server, a product granting no entitlement, or a
        // receipt already gone when this started. The item reached a terminal
        // state without dead-lettering and the server answered.
        return .confirmed
    }
